...
struct cava_plan *cava_init(int number_of_bars, unsigned int rate, int channels,
                            int autosens, double noise_reduction,
  ...
  if (channels < 1 || channels > 2) {
    snprintf(p->error_message, 1024,
             "cava_init called with illegal number of channels: %d, number of "
             "channels "
             "supported are "
             "1 and 2",
             channels);
  ...
  if (rate < 1 || rate > 384000) {
    snprintf(p->error_message, 1024,
             "cava_init called with illegal sample rate: %d\n", rate);
    p->status = -1;
    return p;
  }
  ...
  if (number_of_bars < 1) {
    snprintf(p->error_message, 1024,
             "cava_init called with illegal number of bars: %d, number of "
             "channels must be "
             "positive integer\n",
             number_of_bars);
    p->status = -1;
  ...
  if (number_of_bars > fft_buffer_size / 2 + 1) {
    snprintf(p->error_message, 1024,
             "cava_init called with illegal number of bars: %d, for %d sample "
             "rate number of "
  ...
  if (low_cut_off >= high_cut_off) {
    snprintf(p->error_message, 1024,
             "high_cut_off must be a higher than low_cut_off\n");
    p->status = -1;
    return p;
  }
  if ((unsigned int)high_cut_off > rate / 2) {
    snprintf(p->error_message, 1024,
             "high_cut_off can't be higher than sample rate / 2. (Nyquist "
             "Sampling Theorem)\n");
    p->status = -1;
    return p;
  }
  ...
  p->FFTbuffer_lower_cut_off =
      (int *)malloc((number_of_bars + 1) * sizeof(int));
  ...
  p->prev_cava_out =
      (double *)malloc(number_of_bars * channels * sizeof(double));
  ...
  for (int i = 0; i < p->FFTbassbufferSize; i++) {
    p->bass_multiplier[i] =
        0.5 * (1 - cos(2 * M_PI * i / (p->FFTbassbufferSize - 1)));
  }
  ...
  p->p_bass_l = fftw_plan_dft_r2c_1d(p->FFTbassbufferSize, p->in_bass_l,
  ...
  memset(p->out_bass_l, 0,
         (p->FFTbassbufferSize / 2 + 1) * sizeof(fftw_complex));
  ...
  if (p->audio_channels == 2) {
    ...
    p->p_bass_r = fftw_plan_dft_r2c_1d(p->FFTbassbufferSize, p->in_bass_r,
                                       p->out_bass_r, fftw_flag);
    ...
    p->p_r =
        fftw_plan_dft_r2c_1d(p->FFTbufferSize, p->in_r, p->out_r, fftw_flag);
  ...
  // calculate frequency constant (used to distribute bars across the frequency
  // band)
  double frequency_constant =
      log10((float)lower_cut_off / (float)upper_cut_off) /
      (1 / ((float)p->number_of_bars + 1) - 1);

  float *relative_cut_off =
      (float *)malloc((p->number_of_bars + 1) * sizeof(float));
  ...
  for (int n = 0; n < p->number_of_bars + 1; n++) {
    ...
    p->cut_off_frequency[n] =
        upper_cut_off * pow(10, bar_distribution_coefficient);
    ...
    if (n > 0) {
      if (!first_bar) {
        ...
        // pushing the spectrum up if the exponential function gets "clumped" in
        // the bass and caluclating new cut off frequencies
        if (p->FFTbuffer_lower_cut_off[n] <=
          ...
          if (n < p->bass_cut_off_bar) {
            if (p->FFTbuffer_lower_cut_off[n - 1] + 1 <
                p->FFTbassbufferSize / 2 + 1)
              room_for_more = 1;
          } else {
            if (p->FFTbuffer_lower_cut_off[n - 1] + 1 <
                p->FFTbufferSize / 2 + 1)
          ...
          if (room_for_more) {
            ...
            p->FFTbuffer_lower_cut_off[n] =
                p->FFTbuffer_lower_cut_off[n - 1] + 1;
            p->FFTbuffer_upper_cut_off[n - 1] =
                p->FFTbuffer_lower_cut_off[n] - 1;
          }
        }
  ...
  for (int n = 0; n < p->number_of_bars; n++) {
    ...
    p->eq[n] /=
        p->FFTbuffer_upper_cut_off[n] - p->FFTbuffer_lower_cut_off[n] + 1;
  }
...
void cava_execute(double *cava_in, int new_samples, double *cava_out,
  ...
  if (new_samples > 0) {
    ...
    p->framerate +=
        (double)((p->rate * p->audio_channels * p->frame_skip) / new_samples) /
        64;
    p->frame_skip = 1;
  ...
  for (int n = 0; n < p->number_of_bars; n++) {
    ...
    for (int i = p->FFTbuffer_lower_cut_off[n];
         i <= p->FFTbuffer_upper_cut_off[n]; i++) {
  ...
  int overshoot = 0;
  double gravity_mod =
      pow((60 / p->framerate), 2.5) * 1.54 / p->noise_reduction;
  ...
  for (int n = 0; n < p->number_of_bars * p->audio_channels; n++) {
    ...
    if (cava_out[n] < p->prev_cava_out[n] && p->noise_reduction > 0.1) {
      cava_out[n] = p->cava_peak[n] *
                    (1.0 - (p->cava_fall[n] * p->cava_fall[n] * gravity_mod));
...
JNIEXPORT jfloatArray JNICALL Java_com_karlstav_cava_MyGLRenderer_InitCava(
    JNIEnv *env, jobject thiz, jint number_of_bars_set, jint refresh_rate,
    jint lower_cut_off, jint higher_cut_off) {
  jfloatArray cuttOffFreq = (*env)->NewFloatArray(env, number_of_bars_set + 1);
  float noise_reduction = pow((float)refresh_rate / 130, 0.75);

  plan = cava_init(number_of_bars_set, 44100, 1, 1, noise_reduction,
                   lower_cut_off, higher_cut_off);
...
JNIEXPORT jdoubleArray JNICALL Java_com_karlstav_cava_MyGLRenderer_ExecCava(
    JNIEnv *env, jobject thiz, jdoubleArray cava_input, jint new_samples) {
  ...
  (*env)->SetDoubleArrayRegion(env, cavaReturn, 0, plan->number_of_bars,
                               cava_out);
...
JNIEXPORT int JNICALL Java_com_karlstav_cava_CavaCoreTest_InitCava(
    JNIEnv *env, jobject thiz, jint number_of_bars_set) {

  plan = cava_init(number_of_bars_set, 44100, 1, 1, 0.7, 50, 10000);
  return 1;
}

JNIEXPORT jdoubleArray JNICALL Java_com_karlstav_cava_CavaCoreTest_ExecCava(
    JNIEnv *env, jobject thiz, jdoubleArray cava_input, jint new_samples) {
...
Java_com_karlstav_cava_MyGLRenderer_DestroyCava(JNIEnv *env, jobject thiz) {
  cava_destroy(plan);
}
#endif
