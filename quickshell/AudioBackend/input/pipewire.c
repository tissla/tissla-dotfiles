...
static void on_stream_state_changed(void *_data,
  ...
  case PW_STREAM_STATE_STREAMING:
    pw_loop_update_timer(pw_main_loop_get_loop(data->loop), data->timer, NULL,
                         NULL, false);
    break;
...
void *input_pipewire(void *audiodata) {
  ...
  data.timer =
      pw_loop_add_timer(pw_main_loop_get_loop(data.loop), on_timeout, &data);
  ...
  data.stream = pw_stream_new_simple(pw_main_loop_get_loop(data.loop), "cava",
