...
struct audio_data {
  ...
  int threadparams; // shared variable used to prevent main thread from
                    // cava_init before input threads have finalized parameters
                    // (0=allow cava_init, 1=disallow)
  ...
  int autoconnect;  // auto connect to audio source (0=off, 1=once at startup,
                    // 2=regularly)
